...
from empleado join certificado on empleado.EID = certificado.EID
...
select avion.aid
from avion
where avion.autonomia > (select vuelo.distancia from vuelo where origen = 'Los Angeles' and destino = 'Chicago');

select distinct empleado.nombre
from empleado join certificado on empleado.EID = certificado.EID
join avion on certificado.AID = avion.AID
where avion.autonomia > 3000 and avion.NOMBRE != 'Boeing%';
...
select empleado.nombre
from empleado
where empleado.salario >= all(
select empleado.salario
from empleado
minus select max(salario) from empleado) and empleado.salario != (select MAX(salario) from empleado);


select empleado.EID
from empleado join certificado on empleado.EID = certificado.EID
group by empleado.EID having count(*) = (select max(count (*)) from certificado group by certificado.eid); 


select empleado.EID
from empleado join certificado on empleado.EID = certificado.EID
group by empleado.EID having count(*) = 3;

--------------- 8

select avion.nombre
from avion
where 80000 <= ALL(select empleado.salario 
from empleado join certificado on empleado.eid=certificado.eid 
where certificado.aid = avion.aid);

------------- 9

select empleado.nombre, certificado.EID
from empleado join certificado on empleado.EID = certificado.EID
join avion on certificado.AID = avion.AID
and certificado.EID in ( select certificado.EID 
from certificado
group by certificado.EID having count(*) > 3);

------------------------------


select avion.nombre, sum(salario)
from avion join certificado on avion.AID = certificado.AID
join empleado on certificado.EID = empleado.EID
group by avion.nombre
order by sum(salario) desc;


select vuelo.origen, vuelo.destino, count(distinct empleado.eid), min(empleado.salario)
from vuelo join avion on distancia <= autonomia
join certificado on avion.AID = certificado.AID
join empleado on empleado.EID = certificado.EID
group by vuelo.origen, vuelo.destino;
